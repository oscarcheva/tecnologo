...
TipoRet iguales(Archivo a, char *version1, char *version2);                           // Obligatorio 2
...
int main()
{
    ...
    while (!salir)
    {
        ...
        else if (0 == strcmp(nombre_comando, "iguales"))
        {
            ...
            TipoRet salida = iguales(a, version1, version2);
            if (salida == OK)
                printf("OK\n");
            else if (salida == NO_IMPLEMENTADA)
                printf("NO_IMPLEMENTADA\n");
...
Archivo crearArchivo(char *nombre)
{
    ...
    Archivo nuevoArchivo = crearArchivoVacio(nombre);
...
{
    if (tienePrimerHijoVersion(a, version))
        return ERROR;
    insertarLineaVersionDeArchivo(a, version, linea, nroLinea);
    unsigned int ultimaLinea = numeroUltimaLinea(a, version);
    if (a == NULL || !existeVersionEnArchivo(a, version))
        return ERROR;
    else if ((ultimaLinea != 0 && ultimaLinea < nroLinea - 1) || ultimaLinea == 0)
        return ERROR;
    return OK;
}
...
TipoRet borrarLinea(Archivo &a, char *version, unsigned int nroLinea)
{
    if (a == NULL || !existeVersionEnArchivo(a, version))
        return ERROR;
    else if (numeroUltimaLinea(a, version) < nroLinea)
        return ERROR;
...
TipoRet iguales(Archivo a, char *version1, char *version2)
{
    if (!existeVersionEnArchivo(a, version1))
        return ERROR;
    if (!existeVersionEnArchivo(a, version2))
        return ERROR;
    if (igualesVersionesArchivo(a, version1, version2))
        printf("Versiones iguales\n");
    else
        printf("Versiones no iguales\n");
    return OK;
}
