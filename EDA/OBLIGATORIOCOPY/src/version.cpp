...
void copiarLineasDelPadre(NodoVersion padre, NodoVersion &hijo);
    ...
    if (version != NULL)
    {
        if (num_version[1] != '\0')
        {
            if (version->raizVersion != NULL)
            {
                ...
                hijo->padre = padre;
                copiarLineasDelPadre(padre, hijo);
...
void copiarLineasDelPadre(NodoVersion padre, NodoVersion &hijo)
{
    ...
    {
        Linea actualPadre = padre->inicioLineas;
        while (actualPadre != NULL)
        {

            if (hijo->inicioLineas == NULL)
                hijo->inicioLineas = crearLineaVacia();
            unsigned int numeroLinea = getNumeroLinea(actualPadre);
            char *valorCadenaLinea = obtenerTextoLinea(actualPadre, numeroLinea);
            insertarLinea(hijo->inicioLineas, valorCadenaLinea, numeroLinea);
            delete[] valorCadenaLinea;
            actualPadre = siguienteLinea(actualPadre);
        }
    }
}
...
NodoVersion obtenerRaiz(Version nodoLista, char **numVersion)
{
    ...
    while (actual != NULL && actual->raizVersion != NULL)
    {
...
{
    NodoVersion actual = NULL;
    if (version != NULL)
    {
        actual = obtenerVersion(version, numeroVersion);
        if (actual != NULL)
            return actual->primerHijo != NULL;
    }
    return actual != NULL;
}
...
void imprimirVersion(Version version)
{
    ...
    while (actual != NULL)
    {
        ...
        char *num_version = convertirCadenaArregloChar(version->numero_version);
        agregarCaracteresCadena(versiones, num_version);
        delete[] num_version;
...
void imprimirLineasVersion(Version version, char *numeroVersion)
{
    ...
    if (actual != NULL)
    {
        ...
        else
        {
            ...
            {
                printf("%d - ", getNumeroLinea(lineaActual));
...
void destruirVersion(NodoVersion &version)
{
    ...
    destruirLineas(version->inicioLineas);
...
void mostrarCambiosVersiones(Version version, char *numeroVersion)
{
    ...
    NodoVersion hijo = obtenerVersion(version, numeroVersion);
    NodoVersion padre = hijo->padre;

    cambiosEnLinea(padre->inicioLineas, hijo->inicioLineas);
}

bool versionesIguales(Version version, char *version1, char *version2)
{

    NodoVersion vers1 = obtenerVersion(version, version1);
    NodoVersion vers2 = obtenerVersion(version, version2);

    Linea lineaVersion1 = vers1->inicioLineas;
    Linea lineaVersion2 = vers2->inicioLineas;

    return sonLineasIguales(lineaVersion1, lineaVersion2);
}
